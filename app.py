...
def filter_posts_by_date(posts, date_from, date_to):
    ...
    for p in posts:
        ...
        if pd and d_from and pd < d_from:
            continue
        if pd and d_to and pd > d_to.replace(hour=23, minute=59):
            continue
        filtered.append(p)
    return filtered
...
def search():
    ...
    try:
        ...
        return jsonify({"error": "Search timed out (5 min limit)"}), 504
...
def scraperapi_search():
    ...
    try:
        ...
        return jsonify({"error": "ScraperAPI search timed out (10 min limit)"}), 504
    except Exception as e:
        return jsonify({"error": str(e)}), 500
